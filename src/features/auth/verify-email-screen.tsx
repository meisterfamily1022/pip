...
export function VerifyEmailScreen() {
  ...
  const submit = useCallback(async (): Promise<void> => {
    ...
    try { await verifyEmail(email, code.trim()); await pendingVerification.clear(); router.replace('/parent/home'); }
  ...
  const resend = useCallback(async (): Promise<void> => {
    if (!email) return;
    setError(null);
    try { await resendVerification(email); setNotice('If that address needs confirming, a new code is on its way.'); }
    catch (caught: unknown) { setError(caught instanceof AuthRequestError ? caught.message : 'We could not reach the server. Try again shortly.'); }
  }, [email]);
  return (
    <OnboardingScreen description={email ? `Enter the six-digit code we sent to ${email}.` : 'Enter the six-digit code from your email.'} footer={<PrimaryButton disabled={submitting || code.trim().length !== 6} label={submitting ? 'Confirming…' : 'Confirm email'} onPress={() => void submit()} />} title="Confirm your email">
