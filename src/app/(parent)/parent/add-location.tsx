...
export default function AddLocationRoute() {
  ...
  return <OnboardingScreen title={isStorage ? 'Add storage spot' : 'Add room'} description={isStorage ? `Add a storage spot inside ${roomLabel ?? 'the selected room'}.` : 'Create a room where toys can belong.'} footer={<PrimaryButton label={saving ? 'Saving…' : 'Save'} disabled={saving} onPress={save} />}><BackButton onPress={() => router.back()} /><Field label={isStorage ? 'Storage spot name' : 'Room name'} value={name} onChangeText={(value) => { setName(value); setError(null); }} placeholder={isStorage ? 'Blue Bin' : 'Playroom'} error={error} />{isStorage && roomLabel && <Field label="Room" value={roomLabel} onChangeText={() => {}} />}</OnboardingScreen>;
}
