...
const destinations = [
  { href: '/parent/toy-library' as const, title: 'Toy library', description: 'Add, find, and manage toys', tint: theme.colors.surfaceSage },
  { href: '/parent/locations' as const, title: 'Rooms & storage', description: 'Keep every toy easy to find', tint: theme.colors.surfaceYellow },
  { href: '/parent/select-child' as const, title: 'Child mode', description: 'Choose who is playing', tint: theme.colors.surfacePeach, locks: true },
  { href: '/parent/settings' as const, title: 'Settings', description: 'Children, choices, cleanup, and access', tint: theme.colors.surfaceLavender },
] as const;
...
const styles = StyleSheet.create({ error: { color: theme.colors.error }, intro: { alignItems: 'center', flexDirection: 'row', gap: 18 }, illustration: { backgroundColor: '#F8D8C9', borderRadius: 22, height: 92, overflow: 'hidden', position: 'relative', width: 92 }, arc: { borderColor: theme.colors.coralAction, borderRadius: 50, borderWidth: 8, height: 66, left: 12, position: 'absolute', top: 28, width: 66 }, block: { backgroundColor: theme.colors.surfaceSage, borderRadius: 9, height: 30, left: 38, position: 'absolute', top: 26, width: 30 }, dot: { backgroundColor: theme.colors.surfaceYellow, borderRadius: 12, height: 24, left: 18, position: 'absolute', top: 17, width: 24 }, introCopy: { flex: 1, gap: 4 }, introTitle: { color: theme.colors.primaryText, fontFamily: 'Georgia', fontSize: 21, fontWeight: '700' }, introText: { color: theme.colors.secondaryText, fontSize: 15, lineHeight: 21 }, links: { gap: 10 }, sessionChild: { color: theme.colors.coralDark, fontSize: 14, fontWeight: '800' }, sessionToy: { color: theme.colors.primaryText, fontFamily: 'Georgia', fontSize: 22, fontWeight: '700' } });
