...
export default function ChildHomeRoute() {
  ...
  return <SafeAreaView style={styles.container}><Text accessibilityRole="header" style={styles.title}>{nickname ? `What would you like to play with, ${nickname}?` : 'What would you like to play with?'}</Text><View style={styles.actions}><ChildButton label="Find Something to Play With" onPress={() => router.push('/child/categories')} /><ChildButton label="Surprise Me" onPress={() => router.push({ pathname: '/child/toy-suggestions', params: { category: 'anything', surprise: '1' } })} /><ChildButton label="Current Toy" disabled={!hasActive} onPress={() => router.push('/child/current-toy')} /><ChildButton label="Parent Mode" secondary onPress={() => router.push('../child/parent-return')} /></View></SafeAreaView>;
}
